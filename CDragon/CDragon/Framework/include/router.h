...
class Router {
public:
    void addStaticRoute(const string& path, const handler_function& handler) { staticRoutes.insert({path, handler}); }
...
private:
    ...
    bool handleStaticRoute(const string& path, HttpRequest& request, HttpResponse& response);
    bool handleDynamicRoute(const string& path, HttpRequest& request, HttpResponse& response);
};
