...
using std::string, std::istringstream, std::ostringstream, std::getline, std::cout, std::endl;
using nlohmann::json;
using std::exception;
...
void HttpRequest::parseRawMetaData() {
    if (rawMetaData.empty()) {
        throw std::runtime_error("Парсинг невозможен, так как HttpRequest получил пустые мета даные запроса");
    }
...
void HttpRequest::parseHeaders(istringstream& stream) {
    ...
    while (getline(stream, line)) {
        ...
        if (pos != string::npos) {
            ...
            value = trim(value);    // Убираем ": " и символы [' ', '\r', '\n', '\t']
...
void HttpRequest::parseRawBody() {
    ...
    unordered_map<string, string>::const_iterator pos = headers.find("Content-Type");
...
void HttpRequest::parseJsonData() {
    ...
    try {
        json parsedJson = json::parse(rawBody);

        for (const auto& [key, value] : parsedJson.items()) {
            jsonData.insert({key, value.get<string>()});       // serialization to string
        }
