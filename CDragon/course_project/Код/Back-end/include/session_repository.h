...
using std::chrono::system_clock, std::chrono::time_point,
...
public:
  ...
  bool isSessionValid(const string &sessionID) {
    unordered_map<string, Session>::const_iterator it =
    ...
    auto sessionDuration =
        duration_cast<seconds>(now - session.creationTime).count();
  ...
  const Session &getSessionData(const string &sessionID) {
    ...
    throw runtime_error("Такой session_id в репозитории не существует");
  }

  void setSessionTTL(const int sessionTTL) { this->sessionTTL = sessionTTL; }
